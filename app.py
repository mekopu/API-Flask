...
def register():
    ...
    if mongo.db['user'].find_one({'username': data['username']}):
        ...
        user_id = mongo.db['user'].insert_one({'username': data['username'], 'password': generate_password_hash
        (data['password'])})
...
def login():
    ...
    payload = {'_id': str(user['_id']), 'exp':datetime.utcnow() + timedelta(minutes=30)}
    access_token = jwt.encode(payload, secret_key, algorithm='HS256')
    return jsonify({'access_token': access_token})

@app.route("/api/auth/todo", methods=['POST'])
def add_todo_item():
    try:
        data = request.form.to_dict()
...
@app.route("/api/auth/todo/list_all", methods=['POST'])
def get_todo_list():
    try:
        data = request.form.to_dict()
        data_token = jwt.decode(data['token'], secret_key, algorithms=['HS256'], verify=True)
...
@app.route('/api/auth/todo/update', methods=['POST'])
...
@app.route('/api/auth/todo/delete', methods=['POST'])
